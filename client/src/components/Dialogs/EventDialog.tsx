    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-900 border border-slate-600 rounded-2xl max-w-md w-full p-6 animate-slide shadow-2xl">
        ...
        <div className="text-center mb-5">
          ...
          <h2 className="text-xl font-bold text-slate-100">{eventDef.title}</h2>
          <p className="text-slate-400 text-sm mt-2 leading-relaxed">{eventDef.description}</p>
        </div>
          ...
          {eventDef.choices.map(choice => {
            ...
            return (
              <button
                ...
                `}
              >
                <div className="font-medium text-slate-200">{choice.label}</div>
                <div className="text-sm text-slate-400 mt-0.5">{choice.description}</div>
                ...
                {choice.moneyRequired && !canAfford && (
                  <div className="text-xs text-red-400 mt-1">
                    Þarfnast {choice.moneyRequired.toLocaleString()} kr
                  </div>
                )}
              </button>
            )
